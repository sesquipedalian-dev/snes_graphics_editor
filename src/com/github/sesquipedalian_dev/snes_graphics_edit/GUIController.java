...
public class GUIController {
    ...
    public void menuSave() {
        ...
        String fn = Main.editingData.getFilename();
        if(fn != null && !fn.equals("")) {
            try {
                Main.editingData.toFile(fn);
            } catch(IOException e) {
    ...
    public void menuSaveAs() {
        ...
        if(f != null) {
            try {
                Main.editingData.toFile(f.getAbsolutePath());
                Main.editingData.setFilename(f.getAbsolutePath());

                Alert a = new Alert(Alert.AlertType.INFORMATION);
                a.setTitle("Saved!");
                a.setHeaderText("");
                a.setContentText(String.format("Succesfully saved to %s", Main.editingData.getFilename()));
                a.showAndWait();
            } catch(IOException e) {
                Alert a = new Alert(Alert.AlertType.ERROR);
                a.setTitle("ERROR!");
    ...
    public void newEditingData(int bitDepthSel) {
        ...
        Main.editingData = new EditingData(bitDepth);
    }
}
